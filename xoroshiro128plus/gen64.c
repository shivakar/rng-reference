...
void usage(const char* progName, int exit_val) {
    ...
    fprintf(stderr,
            "\n    -t    : Starting index (starting with 0) of RNG draws to "
            "output");
    fprintf(
        stderr,
    ...
    fprintf(stderr, "\n    -h    : Show this help");
    fprintf(stderr,
            "\n\nIf no options are provided, files for the following seeds and "
            "startIdx");
    fprintf(stderr, "\n            combinations are generated:");
    fprintf(stderr,
            "\n    uint64_t seeds[5] = { 1, 1740, 3812201, 6662751973, "
            "5164531120897553273};");
    fprintf(stderr,
            "\n    uint64_t starts[7] = { 0, 1024, 9118, 674637, 7536456, 1e9, "
            "5e9 };");
    fprintf(stderr,
    ...
    fprintf(stderr,
            "\nWhere 'sample_type' values indicate the following RNG output "
            "ranges:");
    fprintf(stderr, "\n    'uint64': [0, 2^64)");
    fprintf(stderr, "\n    'double_co': [0.0, 1.0)");
    fprintf(stderr, "\n    'double_oc': (0.0, 1.0]");
...
void reset(uint64_t seed) {
    splitmix64_t sm64;
    splitmix64_init(&sm64, seed);
    s[0] = splitmix64_uint64(&sm64);
    s[1] = splitmix64_uint64(&sm64);
}
...
void write_samples_to_file(uint64_t seed, uint64_t num_samples,
    ...
    char filename[255];
    sprintf(filename,
            "xoroshiro128plus-%" PRIu64 "-%s-%" PRIu64 "-%" PRIu64 ".txt", seed,
            rt_str, first_idx, num_samples);
    ...
    for (int i = 0; i < num_seeds; i++) {
        for (int j = 0; j < num_starts; j++) {
            for (int rt = UINT64; rt <= DOUBLE_CO; ++rt) {
                write_samples_to_file(seeds[i], num_samples, starts[j], rt);
            }
        }
    }
